...
import goryachev.memsafecrypto.CIntArrayArray;
...
public class AESEngine
	implements BlockCipher, ICryptoZeroable
{
	// The S box
	...
	private int ROUNDS;
	private CIntArrayArray workingKey;
	...
	*/
	 ...
	 */
	private CIntArrayArray generateWorkingKey(CByteArray key, boolean forEncryption)
	{
		...
		CIntArrayArray W = new CIntArrayArray(ROUNDS + 1, 4); // 4 words in a block

        switch (KC)
        {
        case 4:
	        {
	            int col0 = CUtils.littleEndianToInt(key,  0);     W.set(0, 0, col0);
	            int col1 = CUtils.littleEndianToInt(key,  4);     W.set(0, 1, col1);
	            int col2 = CUtils.littleEndianToInt(key,  8);     W.set(0, 2, col2);
	            int col3 = CUtils.littleEndianToInt(key, 12);     W.set(0, 3, col3);
	
	            for (int i = 1; i <= 10; ++i)
	            {
	                int colx = subWord(shift(col3, 8)) ^ rcon[i - 1];
	                col0 ^= colx;       W.set(i, 0, col0);
	                col1 ^= col0;       W.set(i, 1, col1);
	                col2 ^= col1;       W.set(i, 2, col2);
	                col3 ^= col2;       W.set(i, 3, col3);
	            }
        ...
        case 6:
	        {
	            ...
	            int col4 = CUtils.littleEndianToInt(key, 16);
	            int col5 = CUtils.littleEndianToInt(key, 20);
	            ...
	            for (;;)
	            {
	                ...
	                col0 ^= colx;       W.set(i, 2, col0);
	                col1 ^= col0;       W.set(i, 3, col1);
	
	                col2 ^= col1;       W.set(i + 1, 0, col2);
	                col3 ^= col2;       W.set(i + 1, 1, col3);
	                col4 ^= col3;       W.set(i + 1, 2, col4);
	                col5 ^= col4;       W.set(i + 1, 3, col5);
	
	                colx = subWord(shift(col5, 8)) ^ rcon; rcon <<= 1;
	                col0 ^= colx;       W.set(i + 2, 0, col0);
	                col1 ^= col0;       W.set(i + 2, 1, col1);
	                col2 ^= col1;       W.set(i + 2, 2, col2);
	                col3 ^= col2;       W.set(i + 2, 3, col3);
        ...
        case 8:
	        {
	            ...
	            int col4 = CUtils.littleEndianToInt(key, 16);     W.set(1, 0, col4);
	            int col5 = CUtils.littleEndianToInt(key, 20);     W.set(1, 1, col5);
	            int col6 = CUtils.littleEndianToInt(key, 24);     W.set(1, 2, col6);
	            int col7 = CUtils.littleEndianToInt(key, 28);     W.set(1, 3, col7);
	            ...
	            for (;;)
	            {
	                ...
	                colx = subWord(col3);
	                col4 ^= colx;       W.set(i, 0, col4);
	                col5 ^= col4;       W.set(i, 1, col5);
	                col6 ^= col5;       W.set(i, 2, col6);
	                col7 ^= col6;       W.set(i, 3, col7);
	                ++i;
	            }
    ...
    private void encryptBlock(CByteArray in, int inOff, CByteArray out, int outOff, CIntArrayArray KW)
    {
        int C0 = CUtils.littleEndianToInt(in, inOff +  0);
        int C1 = CUtils.littleEndianToInt(in, inOff +  4);
        int C2 = CUtils.littleEndianToInt(in, inOff +  8);
        int C3 = CUtils.littleEndianToInt(in, inOff + 12);

        int t0 = C0 ^ KW.get(0, 0);
        int t1 = C1 ^ KW.get(0, 1);
        int t2 = C2 ^ KW.get(0, 2);

        int r = 1, r0, r1, r2, r3 = C3 ^ KW.get(0, 3);
        while (r < ROUNDS - 1)
        {
            r0 = T0[t0&255] ^ shift(T0[(t1>>8)&255], 24) ^ shift(T0[(t2>>16)&255], 16) ^ shift(T0[(r3>>24)&255], 8) ^ KW.get(r, 0);
            r1 = T0[t1&255] ^ shift(T0[(t2>>8)&255], 24) ^ shift(T0[(r3>>16)&255], 16) ^ shift(T0[(t0>>24)&255], 8) ^ KW.get(r, 1);
            r2 = T0[t2&255] ^ shift(T0[(r3>>8)&255], 24) ^ shift(T0[(t0>>16)&255], 16) ^ shift(T0[(t1>>24)&255], 8) ^ KW.get(r, 2);
            r3 = T0[r3&255] ^ shift(T0[(t0>>8)&255], 24) ^ shift(T0[(t1>>16)&255], 16) ^ shift(T0[(t2>>24)&255], 8) ^ KW.get(r++, 3);
            t0 = T0[r0&255] ^ shift(T0[(r1>>8)&255], 24) ^ shift(T0[(r2>>16)&255], 16) ^ shift(T0[(r3>>24)&255], 8) ^ KW.get(r, 0);
            t1 = T0[r1&255] ^ shift(T0[(r2>>8)&255], 24) ^ shift(T0[(r3>>16)&255], 16) ^ shift(T0[(r0>>24)&255], 8) ^ KW.get(r, 1);
            t2 = T0[r2&255] ^ shift(T0[(r3>>8)&255], 24) ^ shift(T0[(r0>>16)&255], 16) ^ shift(T0[(r1>>24)&255], 8) ^ KW.get(r, 2);
            r3 = T0[r3&255] ^ shift(T0[(r0>>8)&255], 24) ^ shift(T0[(r1>>16)&255], 16) ^ shift(T0[(r2>>24)&255], 8) ^ KW.get(r++, 3);
        }
        ...
        C0 = (S[r0&255]&255) ^ ((S[(r1>>8)&255]&255)<<8) ^ ((s[(r2>>16)&255]&255)<<16) ^ (s[(r3>>24)&255]<<24) ^ KW.get(r, 0);
        C1 = (s[r1&255]&255) ^ ((S[(r2>>8)&255]&255)<<8) ^ ((S[(r3>>16)&255]&255)<<16) ^ (s[(r0>>24)&255]<<24) ^ KW.get(r, 1);
        C2 = (s[r2&255]&255) ^ ((S[(r3>>8)&255]&255)<<8) ^ ((S[(r0>>16)&255]&255)<<16) ^ (S[(r1>>24)&255]<<24) ^ KW.get(r, 2);
        C3 = (s[r3&255]&255) ^ ((s[(r0>>8)&255]&255)<<8) ^ ((s[(r1>>16)&255]&255)<<16) ^ (S[(r2>>24)&255]<<24) ^ KW.get(r, 3);

        CUtils.intToLittleEndian(C0, out, outOff +  0);
        CUtils.intToLittleEndian(C1, out, outOff +  4);
        CUtils.intToLittleEndian(C2, out, outOff +  8);
        CUtils.intToLittleEndian(C3, out, outOff + 12);
    }
    

    private void decryptBlock(CByteArray in, int inOff, CByteArray out, int outOff, CIntArrayArray KW)
    {
        int C0 = CUtils.littleEndianToInt(in, inOff +  0);
        int C1 = CUtils.littleEndianToInt(in, inOff +  4);
        int C2 = CUtils.littleEndianToInt(in, inOff +  8);
        ...
        int t0 = C0 ^ KW.get(ROUNDS, 0);
        int t1 = C1 ^ KW.get(ROUNDS, 1);
        int t2 = C2 ^ KW.get(ROUNDS, 2);

        int r = ROUNDS - 1, r0, r1, r2, r3 = C3 ^ KW.get(ROUNDS, 3);
        while (r > 1)
        {
            r0 = Tinv0[t0&255] ^ shift(Tinv0[(r3>>8)&255], 24) ^ shift(Tinv0[(t2>>16)&255], 16) ^ shift(Tinv0[(t1>>24)&255], 8) ^ KW.get(r, 0);
            r1 = Tinv0[t1&255] ^ shift(Tinv0[(t0>>8)&255], 24) ^ shift(Tinv0[(r3>>16)&255], 16) ^ shift(Tinv0[(t2>>24)&255], 8) ^ KW.get(r, 1);
            r2 = Tinv0[t2&255] ^ shift(Tinv0[(t1>>8)&255], 24) ^ shift(Tinv0[(t0>>16)&255], 16) ^ shift(Tinv0[(r3>>24)&255], 8) ^ KW.get(r, 2);
            r3 = Tinv0[r3&255] ^ shift(Tinv0[(t2>>8)&255], 24) ^ shift(Tinv0[(t1>>16)&255], 16) ^ shift(Tinv0[(t0>>24)&255], 8) ^ KW.get(r--, 3);
            t0 = Tinv0[r0&255] ^ shift(Tinv0[(r3>>8)&255], 24) ^ shift(Tinv0[(r2>>16)&255], 16) ^ shift(Tinv0[(r1>>24)&255], 8) ^ KW.get(r, 0);
            t1 = Tinv0[r1&255] ^ shift(Tinv0[(r0>>8)&255], 24) ^ shift(Tinv0[(r3>>16)&255], 16) ^ shift(Tinv0[(r2>>24)&255], 8) ^ KW.get(r, 1);
            t2 = Tinv0[r2&255] ^ shift(Tinv0[(r1>>8)&255], 24) ^ shift(Tinv0[(r0>>16)&255], 16) ^ shift(Tinv0[(r3>>24)&255], 8) ^ KW.get(r, 2);
            r3 = Tinv0[r3&255] ^ shift(Tinv0[(r2>>8)&255], 24) ^ shift(Tinv0[(r1>>16)&255], 16) ^ shift(Tinv0[(r0>>24)&255], 8) ^ KW.get(r--, 3);
        }
        ...
        C0 = (Si[r0&255]&255) ^ ((s[(r3>>8)&255]&255)<<8) ^ ((s[(r2>>16)&255]&255)<<16) ^ (Si[(r1>>24)&255]<<24) ^ KW.get(0, 0);
        C1 = (s[r1&255]&255) ^ ((s[(r0>>8)&255]&255)<<8) ^ ((Si[(r3>>16)&255]&255)<<16) ^ (s[(r2>>24)&255]<<24) ^ KW.get(0, 1);
        C2 = (s[r2&255]&255) ^ ((Si[(r1>>8)&255]&255)<<8) ^ ((Si[(r0>>16)&255]&255)<<16) ^ (s[(r3>>24)&255]<<24) ^ KW.get(0, 2);
        C3 = (Si[r3&255]&255) ^ ((s[(r2>>8)&255]&255)<<8) ^ ((s[(r1>>16)&255]&255)<<16) ^ (s[(r0>>24)&255]<<24) ^ KW.get(0, 3);
